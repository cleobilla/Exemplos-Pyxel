...
class Personagem:
    def __init__(self,x,y,largura,altura,cor):
        ...
        self.colImagem = 0
        self.linImagem = 0
        self.frame = 0       # conta quantos frames (updates) já passaram
        self.frameRate = 3  # troca a coluna a cada 4 frames (updates)
        self.definirBox()
    ...
    def mover(self,dx,dy):
        if dy > 0:
            self.linImagem = 0   # baixo (frente)
        elif dy < 0:
            self.linImagem = 1   # cima (costas)
        elif dx < 0:
            self.linImagem = 2   # esquerda
        elif dx > 0:
            self.linImagem = 3   # direita

        # controle da velocidade da animação pela frameRate
        if dx != 0 or dy != 0:
            self.frame += 1
            if self.frame >= self.frameRate:
                self.frame = 0
                self.colImagem = (self.colImagem + 1) % 4
    ...
    def draw(self):
        xImagem = self.colImagem * self.largura # Calcula a posição da imagem na matriz de imagens
        yImagem = self.linImagem * self.altura  # Calcula a posição da imagem na matriz de imagens

        pyxel.blt(self.x1, self.y1,             # Posição na tela
                  0,                            # qual das imagens (matrizes) que pyxel disponibiliza: 0, 1 ou 2.
                  xImagem,yImagem,              # posição da imagem na matriz
